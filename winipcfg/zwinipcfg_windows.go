...
// https://learn.microsoft.com/zh-cn/windows/win32/api/iphlpapi/nf-iphlpapi-getbestroute
func GetBestRoute(destAddr, sourceAddr uint32, bestRoute *MIB_IPFORWARDROW) (ret error) {
	addr := procgetBestRoute.Addr()
	fmt.Println(addr)
	r0, _, _ := syscall.Syscall(procgetBestRoute.Addr(), 3, uintptr(destAddr), uintptr(sourceAddr), uintptr(unsafe.Pointer(bestRoute)))
	if r0 != 0 {
		ret = syscall.Errno(r0)
	}
	return
...
func GtIfEntry2(row *MibIfRow2) (ret error) {
	r0, _, _ := syscall.Syscall(procGetIfEntry2.Addr(), 1, uintptr(unsafe.Pointer(row)), 0, 0)
	if r0 != 0 {
		ret = syscall.Errno(r0)
	}
